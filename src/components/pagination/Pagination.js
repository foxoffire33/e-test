...
export default function Pagination({ resource, currentPage, onPageChange }) {

    const data = resource.data.read();

    const MAX_PAGE_LINKS = 10; // maximum number of page links to show

    let startPageNumber = Math.max(data.meta.pagination.currentPage - Math.floor(MAX_PAGE_LINKS / 2), 1);
    let endPageNumber = Math.min(startPageNumber + MAX_PAGE_LINKS - 1, data.meta.pagination.totalPages);
    ...
    console.log(resource.requestState);

    useEffect(() => {
        resource.reload();
     }, [resource.requestState]);
    ...
    return (<>
        <div className="tw-lg:tw-w-3/5 tw-w-full tw-flex tw-items-center tw-justify-between tw-border-t tw-border-gray-200">
            <div
                ...
                onClick={() => { if (data.meta.pagination.currentPage > 1) { resource.setRequestState({ ...resource.requestState, page: resource.requestState.page - 1 }) } }}
            >
            ...
            <div className="tw-sm:flex tw-inline">
                {pageNumbers.map((pageNumber) => (
                    <p
                        key={pageNumber}
                        onClick={() => resource.setRequestState({ ...resource.requestState, page: pageNumber })}
                        className={`tw-text-sm tw-font-medium tw-leading-none tw-cursor-pointer tw-pt-3 tw-mr-4 tw-px-2 ${resource.requestState.page !== data.meta.pagination.currentPage ? 'tw-text-gray-600 tw-hover:tw-text-gray-700 tw-border-t tw-border-transparent tw-hover:tw-border-gray-400' : 'tw-text-gray-700 tw-border-t tw-border-transparent tw-border-gray-400'
                            } tw-inline`}
                    >{pageNumber}</p>
                ))}
            </div>
            <div
                ...
                onClick={() => { if (data.meta.pagination.currentPage !== data.meta.pagination.totalPages) { resource.setRequestState({ ...resource.requestState, page: resource.requestState.page + 1 }) } }}
            >
